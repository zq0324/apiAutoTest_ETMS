...
class DataProcess:
    ...
    @classmethod
    def handler_files(cls, file_obj: dict, data: dict) -> object:
        """file对象处理方法
        ...
        if file_obj:
            for k, v in file_obj.items():
                # 单文件上传
                files = cls.set_file([k, v], data)
            allure_step('上传文件', str(file_obj))
            return files
    ...
    @classmethod
    def set_file(cls, file_data, data):
        """文件上传
        :param file_data: 字典格式{key：文件路径}
        :param data: data 数据字典格式
        :return 上传文件接口的所需的data,header
        """
        import os
        headers = {}
        key, file = file_data
        file = os.path.join(ReadFile.read_config('$.file_path.test_case'), file)
        if file.strip() != '':
            if os.path.isfile(file):
                data, headers["Content-Type"] = cls.encode_file_data(file, key, data)
                return data, headers
            else:
                logger.error('File %s is not exists' % file)

    @classmethod
    def encode_file_data(cls, file_path, key_name, data):
        """文件编码formdata格式数据"""
